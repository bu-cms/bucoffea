...
def data_mc_after_kfac():
    ...
    for year in [2017,2018]:
        ...
        mc = re.compile(f'(EW.*|TTJets.*FXFX.*|Diboson.*|ST.*|QCD_HT.*|.*DYJetsToLL_M-50_HT_MLM.*){year}')
        region='cr_2m_j'
        ...
        data = re.compile(f'MET_{year}')
        mc = re.compile(f'(TTJets.*FXFX.*|Diboson.*|ST.*|QCD_HT.*|.*DYJetsToLL_M-50_HT_MLM.*|W.*HT.*).*{year}')
        region='cr_1m_j'
        for distribution in ['recoil','ak4_pt0']:
            make_plot(
                      acc,
