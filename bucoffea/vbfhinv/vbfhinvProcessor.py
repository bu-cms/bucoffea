...
class vbfhinvProcessor(processor.ProcessorABC):
    ...
    def process(self, df):
        ...
        for region, cuts in regions.items():
            ...
            # Leading ak4
            w_diak4 = weight_shape(diak4.pt[mask], region_weights.partial_weight(exclude=exclude)[mask])
            ...
            w_btag = weight_shape(btag[mask], region_weights.partial_weight(exclude=exclude)[mask])
            ...
            ezfill('dphijj',             dphi=df["dphijj"][mask],   weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ezfill('detajj',             deta=df["detajj"][mask],   weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ezfill('mjj',                mjj=df["mjj"][mask],      weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ...
            # Two dimensional
            ezfill('recoil_mjj',         recoil=df["recoil_pt"][mask], mjj=df["mjj"][mask], weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ...
            # Tau
            if 'no_veto' in region:
                w_all_taus = weight_shape(taus.pt[mask], region_weights.partial_weight(exclude=exclude)[mask])
